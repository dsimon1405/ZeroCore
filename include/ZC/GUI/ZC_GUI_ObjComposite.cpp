...
// {
//     for (auto pObj : objs) pObj->VConf_GetBordersAndObjsCount_Obj(rBordersCount, rObjsCount);
// }

// void ZC_GUI_ObjComposite::VConf_GetData_Obj(std::vector<ZC_GUI_Border>& rBorder, std::vector<ZC_Vec2<float>>& rBLs,
//     std::vector<ZC_GUI_ObjData>& rObjDatas, int borderIndex)
// {
//     for (auto pObj : objs) pObj->VConf_GetData_Obj(rBorder, rBLs, rObjDatas, borderIndex);
// }
...
//     for (auto pObj : objs)
//     {
//         if (!(pObj->SetObjHolder(_pObjHolder)))   //  if set pObjHolder fails in one of obj, set to already setted pObjHolder = nullptr
//         {
//             for (auto pObj1 : objs)
//             {
//                 if (pObj == pObj1) return false;    //  found obj on wich fail set pObjHolder, may out
//                 pObj1->SetObjHolder(nullptr);
//             }
//         }
//     }
