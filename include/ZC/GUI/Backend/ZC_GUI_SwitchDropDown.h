...
{
    ZC_GUI_SwitchDropDown(const ZC_GUI_Font* pFont, const std::vector<std::wstring>& variants, uint active_variant, float width, float height, ZC_Function<void(uint)>&& _callback,
...
private:
    ...
    float CalculateWidth(const ZC_GUI_Font* pFont, const std::vector<std::wstring>& variants, float width, const ZC_GUI_ColorsButton& colorsButton, uint text_color);
