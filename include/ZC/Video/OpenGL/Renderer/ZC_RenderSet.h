...
struct ZC_RenderSet
{
    typedef typename ZC_RSController::RenderSet ZC_RSC_RenderSet;
    ...
    std::forward_list<ZC_RSC_RenderSet> renderSets;      //  base, for copy in ZC_RSController

    ZC_RenderSet(ZC_ShP_ShPInitSet* pShPInitSet, ZC_VAO&& _vao, ZC_uptr<ZC_GLDraw>&& _upDraw, std::forward_list<ZC_Buffer>&& _buffers,
        std::forward_list<ZC_TexturesSet>&& _texSets = {}, std::forward_list<ZC_RSC_RenderSet> _renderSets = { { ZC_FB_Default } });

    static ZC_uptr<ZC_RenderSet> CreateUptr(ZC_ShP_ShPInitSet* pShPInitSet, ZC_VAO&& _vao, ZC_uptr<ZC_GLDraw>&& _upDraw, std::forward_list<ZC_Buffer>&& _buffers,
        std::forward_list<ZC_TexturesSet>&& _texSets = {}, std::forward_list<ZC_RSC_RenderSet> _renderSets = { { ZC_FB_Default } });

    static ZC_sptr<ZC_RenderSet> CreateShptr(ZC_ShP_ShPInitSet* pShPInitSet, ZC_VAO&& _vao, ZC_uptr<ZC_GLDraw>&& _upDraw, std::forward_list<ZC_Buffer>&& _buffers,
        std::forward_list<ZC_TexturesSet>&& _texSets = {}, std::forward_list<ZC_RSC_RenderSet> _renderSets = { { ZC_FB_Default } });
