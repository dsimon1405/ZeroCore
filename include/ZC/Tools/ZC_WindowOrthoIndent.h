...
enum ZC_WindowOrthoIndentFlag
{
    ...
    ZC_WOIF__X_Center_Pixel     = 1 << 5,   //  takes pixel indent of the heir's window center from center of the system window. Range of indentX 0 is center, negative (closer to left border), positive (closer to right border).

    ZC_WOIF__Y_Top_Pixel        = 1 << 6,   //  takes pixel indent from top system window border.
    ZC_WOIF__Y_Top_Percent      = 1 << 7,   //  takes percent indent from top system window border.
    ZC_WOIF__Y_Bottom_Pixel     = 1 << 8,   //  takes pixel indent from bottom system window border.
    ZC_WOIF__Y_Bottom_Percent   = 1 << 9,   //  takes percent indent from bottom system window border.
    ZC_WOIF__Y_Center           = 1 << 10,  //  indentY no metter. Makes position vertical centered.
    ZC_WOIF__Y_Center_Pixel     = 1 << 11,  //  takes pixel indent of the heir's window center from center of the system window. Range of indentX 0 is center, negative (closer to left border), positive (closer to right border).
};
...
struct ZC_WOIData
{
    ...
    - ZC_WOIF__X_Center_Pixel -> may be negative and positive.
    */
    float indentX;
    /*
    ...
    - ZC_WOIF__Y_Center_Pixel -> may be negative and positive.
    */
    float indentY;
    /*
...
/*
Parent class for calculation position bottom left corner of the quad (window for example) created into the system window.
If system window resizes, recalculates and call virtual method VChanged_bl_WOI() if corner's position was changed.
*/
...
public:
    ...
    //  Set new indent params and calls CalculateCurrentIndents_WOI().
    void SetNewIndentParams_WOI(float _indentX, float _indentY, ZC_WindowOrthoIndentFlags _indentFlags);
    //  Return indent params.
    void GetIndentParams_WOI(float& _indentX, float& _indentY, ZC_WindowOrthoIndentFlags& _indentFlags);
    //  Returns vector of X, Y indents.
    ZC_Vec2<float> GetBL_WOI();
    //  update bl and recalculate the indents from bl for ZC_WOIF__X_Center_Pixel | ZC_WOIF__Y_Center_Pixel mask.
    void CalculateCenterPixelIndentsFromBL_WOI(const ZC_Vec2<float>& bl);

protected:
    ZC_WOIData woiData;
    ZC_Vec2<float> bl_WOI;  //  position of the bottom left corener of the heir into the system window
    ...
    */
    ...
    //  Set new objects width and height. If bl_WOI was changed calls VChanged_bl_WOI().
    void SetNewSize_WOI(float _width, float _height);
    //  Calculate currentIndentX, currentIndentY. Must be called to calculate indents onstart. If bl_WOI was changed calls VChanged_bl_WOI().
    void CalculateCurrentIndents_WOI();

private:
    bool is_Y_ZeroPointOnTop;
    ZC_EC ecZC_WindowResized;

    //  override heirs who need. Calls if bl_WOI was changed.
...
protected:
    ...
    */
    ...
    bool SetNewSize_WOI(float _width, float _height);
