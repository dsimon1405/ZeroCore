...
{
    uint colorUint = ZC_Pack_Float_To_UInt_2x10x10x10(red, green, blue);
    UpdateColor(colorUint);
}

void ZC_TextData::SetColorUChar(uchar red, uchar green, uchar blue)
{
    uint colorUint = ZC_Pack_UChar_To_UInt_2x10x10x10(red, green, blue);
    UpdateColor(colorUint);
}
...
ZC_DrawElements ZC_TextData::CalculateAndSetTextData(ZC_Buffer& rVBO, ZC_Buffer& rEBO, const std::wstring& text, ZC_TextAlignment alignment)
{
    ...
    ul_zc elementsCount = 0ul;
    GLenum elementsType = 0u;
    ZC_DA<uchar> elements = ZC_Buffer::GetTriangleElements(elementsCount, elementsType, static_cast<ul_zc>(pointsCoords.size() / 4ul), 0ul);
