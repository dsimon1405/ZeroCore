...
ZC_Shader* ZC_ShVertex1::GetShader(Name name)    //  add here new
{
    ...
    case Name::game_particle_system: path = ZC_FSPath(shadersPath).append("Game/particle_system.vs").string(); break;
    case Name::game_cube_map: path = ZC_FSPath(shadersPath).append("Game/cube_map.vs").string(); break;
    case Name::game_cylinder_dmg: path = ZC_FSPath(shadersPath).append("Game/cylinder_dmg.vs").string(); break;
...
std::vector<ZC_uptr<ZC_Uniform>> ZC_ShVertex1::GetUniformData(Name name)    //  add here new
{
    ...
    case Name::game_particle_system: return {};
    // case Name::game_particle_system: return ZC_Uniform::GetUniformVector({ G_UN_unData, true });
    case Name::game_cube_map: return {};
    case Name::game_cylinder_dmg: return ZC_Uniform::GetUniformVector(ZC_Uniform::NameType{ .name = ZC_UN_unModel, .isPointer = true });
