...
ZC_Shader* ZC_ShFragment1::GetShader(Name name)    //  add here new
{
    ...
    case Name::game_flame: path = ZC_FSPath(shadersPath).append("Game/flame.fs").string(); break;
...
typename ZC_ShFragment1::Set ZC_ShFragment1::GetVAOAndUniformData(Name name)    //  add here new
{
    ...
    //     .uniforms = ZC_Uniform::GetUniformVector({ .name = ZC_UN_unAlpha, .isPointer = true }) };
    case Name::game_flame: return { GetShader(name), { { new TName[]{ TName::texColor }, 1 } }, {} };
